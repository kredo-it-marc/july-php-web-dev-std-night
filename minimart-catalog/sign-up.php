...
<?php
    function createUser($first_name, $last_name, $username, $password)
    {
        ...
        $result = $conn->query($sql); //run the sql statement. returns TRUE if the sql statement runs successfully, otherwise, FALSE.

        if($result) //check if the insert is successful
        {
        ...
        else
        {
            ...
            echo "<div class='alert alert-danger w-50 mx-auto text-center'>Sign-up failed. Kindly try again. <small>".$conn->error."</small></div>";
        }
